...
> **修复进展（2026-09-03）**：第五节逻辑问题 2、3 已修复；第四节 P1 中"Python 内部 DEFAULT_BASE_URL 重复"已合并（transport.py 为唯一字面量）；P1"provider 识别靠 URL 嗅探"已完成端到端改造——`provider_family` 从 Rust `TranslationInput` 经 translate stage spec 显式传递至 Python `TranslationExecutionRequest`，执行计划优先使用显式值，URL 嗅探降级为兜底。P2"私有函数跨模块导入"已收敛：生产代码 59 处全部清零（AST 校验），涉及 11 个叶模块的 31 个函数提升为公开 API；删除零消费者的死 facade `batch_plan.py`；架构门禁的私有导入例外表已清空，后续任何新的跨模块私有导入将直接报错。P2"半途迁移"已收尾：删除纯透传层 `runtime/pipeline/translation_stage.py`（连同已清空的 `runtime/` 目录树），translate-only 入口直接经 `services.translation.public` 门面构造请求执行；`stages.py → phases/` 迁移完成，垫片删除，`book_flow.py` 与测试直指 phases；层规则与文档（6 处 README/架构文档）同步更新。验证：Python 613 测试全过、架构门禁通过、clippy 无新增告警。P3 已完成：Rust 侧 14 处运行时正则全部改为模块级 `LazyLock` 一次性编译（paddle_markdown 6 处、agent 5 处、downloads/markdown 3 处）；clippy 全目标（lib + bins + tests）存量告警清零——机械项由 `cargo clippy --fix` 自动修复，手工修复含 `from_str` 改名 `from_slug`（3 个调用点）、`metrics` 复杂类型抽 `NativeStats` 别名、`catalog` unwrap 改 if-let、布尔表达式简化等，12 处 `too_many_arguments` 以 `#[allow]` 收口（参数即运行时上下文的扁平透传，收拢结构体收益低）；顺带做了一次性全 crate `rustfmt` 归一化。CI 门禁：`rust-api-architecture.yml` 新增 `cargo fmt --check` 与 `cargo clippy --all-targets -- -D warnings` 两步。`check_architecture.py` 修复 macOS 网络卷 `._*` AppleDouble 元数据文件导致的 UTF-8 解码崩溃。验证：`cargo clippy --all-targets -- -D warnings` 零告警、Rust 421 测试全过（1 个超时断言类测试偶发，单独跑与重跑均通过）、Python 管线门禁通过。**新发现（存量问题，非本轮引入）**：`check_architecture.py` 在 HEAD（含重构前基线 4942e84d）上有 8 条违规——`routes/ai.rs`（直连 state.db/config + 直接 import services）、`routes/metrics.rs`（直连 state.db）、`routes/translate.rs`（直接 import `translate_text`）、`presentation/helpers.rs` 与 `book_projection/metadata.rs`（绕过 models 门面导入 `TranslationLanguageMeta`）、`ocr_flow/text_layer.rs`（越过 mod.rs 直接回交 `execute_process_job`）。**该 8 条已于当日全部修复**：新增路由门面 `services/ai_api.rs`（凭据合并 + ask 编排入口）与 `services/translate_api.rs`，路由经门面触达服务；`routes/common.rs` 新增 `build_ai_route_deps`（SSE 路径改为只克隆 db/data_root/ai 配置三个 owned 句柄，不再整体 clone AppState）与 `build_metrics_route_deps`（复用 health deps）；`TranslationLanguageMeta` 改经 `models::request` 门面导出；`text_layer` 拆出 `TextLayerPreparation`（Ready/Terminal），回交 `execute_process_job` 的决策归位编排层 `ocr_flow/mod.rs`；门禁白名单登记两个新门面前缀。验证：Rust 架构门禁由红转绿、`cargo clippy --all-targets -- -D warnings` 零告警、`cargo fmt --check` 通过、Rust 421 测试全过、Python 管线门禁通过。P1 剩余项已完成：Rust 侧默认值收敛到单一定义点 `config/defaults.rs`（DeepSeek base/balance URL、本地 LLM 模型与 base_url 四个常量，provider.rs/ai.rs/env_vars.rs/reader_ai config 全部改为引用）；Python devtools 残留字面量（job_debug_runner、replay_translation_item）改为从 `deepseek/transport.py` 导入 `DEFAULT_BASE_URL`；新增双端一致性门禁 `devtools/check_default_parity.py`——解析两端定义行直接比对（不内置第三份拷贝），并扫描全部生产代码禁止字面量出现在非定义点（Rust 剥离 cfg(test) 段并排除 api_tests/tests.rs，Python 排除测试目录），已接入 CI `rust-api-architecture.yml`。验证：三门禁全绿、Python 613 测试全过、Rust 421 测试全过、clippy 零告警、fmt 通过。
