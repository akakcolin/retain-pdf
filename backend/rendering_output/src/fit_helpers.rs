//! Port of `output/typst/fit_helpers.py`: the Typst `pdftr_fit_*` helper
//! source lines, verbatim.

pub const PAGE_SPEC_FIT_HELPERS: &[&str] = &[
...
pub const RENDER_BLOCK_FIT_HELPERS: &[&str] = &[
    "#let pdftr_fit_size(lo, hi, eps, fits) = {",
    "  if hi - lo <= eps {",
    "    lo",
    "  } else {",
...
pub fn page_spec_fit_helpers() -> &'static [&'static str] {
    PAGE_SPEC_FIT_HELPERS
}

/// `render_block_fit_helpers()`.
pub fn render_block_fit_helpers() -> &'static [&'static str] {
    RENDER_BLOCK_FIT_HELPERS
}
...
mod tests {
    ...
    #[test]
    fn page_spec_matches_production() {
        let joined = page_spec_fit_helpers().join("\n");
        assert!(joined.starts_with("#let pdftr_fit_size(lo, hi, eps, fits) = {"));
        assert!(joined.contains("cmarker.render(markdown, math: mitex)"));
        assert_eq!(page_spec_fit_helpers().len(), 60);
    }

    #[test]
    fn render_block_matches_production() {
        let joined = render_block_fit_helpers().join("\n");
        assert!(joined.contains("#let pdftr_fit_leading(lo, hi, eps, fits) = {"));
        assert!(joined.contains("pdftr_fit_leading(leading_floor, leading_cap, 0.01em,"));
        assert_eq!(render_block_fit_helpers().len(), 96);
    }
}
