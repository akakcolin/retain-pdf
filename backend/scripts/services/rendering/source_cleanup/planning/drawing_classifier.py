...
def bboxlog_path_blocks_text_strip(kind: str, rect: Rect) -> bool:
    if kind.startswith("stroke-") and "path" in kind:
        return True
    return is_text_like_fill_path(kind, rect)


def is_text_like_fill_path(kind: str, rect: Rect | None) -> bool:
...
def rect_is_text_like_fill_path(rect: Rect | None) -> bool:
